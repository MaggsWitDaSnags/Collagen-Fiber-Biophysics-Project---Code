...
def fibers(count):
    for i in range(count):
        ...
        #Vector matrices that define the change in in position realtive to the 3 axis 
        Rx = np.array([[1,0,0], [0, np.cos(x), -np.sin(x)], [0, np.sin(x), np.cos(x)]]) 
        Ry = np.array([[np.cos(y), 0, np.sin(y)], [0,1,0], [-np.sin(y), 0, np.cos(y)]])
        Rz = np.array([[np.cos(z), -np.sin(z), 0], [-np.sin(z), np.cos(z), 0], [0,0,1]])
        
        Rotation_M = Rx @ Ry @ Rz #rotation matrix, @ = matrix multiplcation
        
        pos = (pos @ Rotation_M.T) + (i*40e-6) #Transposing matrix each iteration to generate rotation 
        ...
        #inital position
        ax.plot(initial_pos[:, 0] * scale, initial_pos[:, 1] * scale, initial_pos[:, 2] * scale, color='black', linestyle='--', alpha=0.4, label=f'Fiber {i+1} Initial')
        ax.scatter(initial_pos[:, 0] * scale, initial_pos[:, 1] * scale, initial_pos[:, 2] * scale, color='gray', s=20, alpha=0.4)

        #final position
        ax.plot(pos[:, 0] * scale, pos[:, 1] * scale, pos[:, 2] * scale, color='blue', label=f'Fiber {i+1} Final')
        ax.scatter(pos[:, 0] * scale, pos[:, 1] * scale, pos[:, 2] * scale, color='darkorange', s=30, zorder=5)
