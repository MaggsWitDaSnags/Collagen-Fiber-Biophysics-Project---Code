...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  2 10:43:41 2026
...
N = 5 #Number of beads
l = 100e-6 #Length of fiber's in between beads (m)
...
(pos[:,0]) = np.deg2rad(np.arange(N)) * l *(2/3.4) * 1e2 #updating the X coords
# "(2/3.4) * 1e2", These values correct for adding in the crimping
# without them the intial state is linear
#"np.deg2rad" because in the next arugument, it's assumed that the argument is in degrees (np.sin)
#So, we set it to radians now

pos[:, 1] = np.sin((pos[:, 0] * -7e5)) *2e-6
